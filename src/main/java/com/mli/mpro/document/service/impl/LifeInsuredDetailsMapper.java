...
@Service
public class LifeInsuredDetailsMapper {

    private static final Logger logger = LoggerFactory.getLogger(LifeInsuredDetailsMapper.class);
    @Autowired
	...
	try {
		...
		String previousPregComp = "";
		String prevPregCompDetails = "";
		boolean isPregnant = false;
	
		// handle NULL pointer check here for MarriageDetails Object
	    if (Objects.nonNull(proposerDetails.getMarriageDetails())) {
	    	if(isNeoOrAggregator){
	    		previousPregComp = getPreviousPregnancyComplicationValue(proposerDetails.getMarriageDetails());
	    		prevPregCompDetails = StringUtils.isNotBlank(proposerDetails.getMarriageDetails().getPrevPregCompDetails())
						? proposerDetails.getMarriageDetails().getPrevPregCompDetails() : "";
			}
	    	if(!isNeoOrAggregator) {
		...
		if (isSwpWli) {
			insuredTotalSumAssured = StringUtils.isEmpty(insuredTotalSumAssured) ? "NA" : insuredTotalSumAssured;
			insuredTotalSumAssuredCI = StringUtils.isEmpty(insuredTotalSumAssuredCI) ? "NA" : insuredTotalSumAssuredCI;
		}else if(isSJB && AppConstants.DEPENDENT.equalsIgnoreCase(formType)){
		...
		dataVariables.put("preAnyComplications", previousPregComp);
		dataVariables.put("preComplicationDetails",prevPregCompDetails);
	...
	private String getPreviousPregnancyComplicationValue(MarriageDetails marriageDetails) {
		if (marriageDetails != null && StringUtils.isNotBlank(marriageDetails.getPreviousPregComp())) {
			switch (marriageDetails.getPreviousPregComp()) {
				case NEO_YES:
					return YES;
				case NEO_NO:
					return NO;
				case STATUS_OTHER_THAN_01_02:
					return NA;
				default:
					return "";
			}
