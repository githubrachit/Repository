 ...
 */
@Service
public class TermMedicalDetailsMapper {
	@Autowired
	private BaseMapper baseMapper;
    private static final Logger logger = LoggerFactory.getLogger(TermMedicalDetailsMapper.class);
	...
	try {
		...
		boolean isProposerApplicable = proposerFormFlag;
        isStep = AppConstants.STEP.equalsIgnoreCase(proposalDetails.getProductDetails().get(0).getProductInfo().getProductId());
        ...
        List<PosvQuestion> posvQuestionsList =(List<PosvQuestion>) Utility.evaluateConditionalOperation(proposalDetails.getPosvDetails() != null ,proposalDetails.getPosvDetails(),"posvQuestions", new ArrayList<>());
		...
		dataForDocument.put("isStep", isStep);
		...
		dataForDocument.put("isProposerApplicable", isProposerApplicable);
		...
		setWeightChangeQuestionData(proposalDetails, dataForDocument);
	...
	private MedicalQuestions setPosvSWPMedicalQuestions(MedicalQuestions medicalQuestions, BaseMapper baseMapper, Map<String, List<PosvQuestion>> posvQAMap, Map<String, List<PosvQuestion>> posvQA12Map , boolean sspJointFlag) {
		...
		//Medical DHU changes
		medicalQuestions.setQuestion19CChoice(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H28C", "H28C1"));
		medicalQuestions.setQuestion19CChoice(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H28C", "H28C2"));
		medicalQuestions.setQuestion19CChoice(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H28C", "H28C3"));
		medicalQuestions.setQuestion19CChoice(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H28C", "H28C4"));
		...
		medicalQuestions.setQuestion30JChoice(Utility.getArrayAns("H26","H26A6",baseMapper,posvQAMap));
		medicalQuestions.setQuestion30KChoice(Utility.getArrayAnsYesOrNoResponse(medicalQuestions.getQuestion30JChoice()));
		medicalQuestions.setQuestionWopTobaccoAnswer(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H36", "H36A"));
		medicalQuestions.setQuestionWopAlcohol1Answer(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H36B", "H36Bi"));
		medicalQuestions.setQuestionWopAlcohol2Answer(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H36B", "H36Bii"));
		medicalQuestions.setQuestionWopAlcoholAnswer(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H36", "H36B"));
		medicalQuestions.setQuestionWopDrugsAnswer(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H36", "H36C"));
		medicalQuestions.setQuestionWopSmokingAnswer(baseMapper.getMedicalQuestionAnswer(posvQAMap, "NA", "H36Ai"));
	...
	private MedicalQuestions setPosvGeneralMedicalQuestions(MedicalQuestions medicalQuestions, BaseMapper baseMapper, Map<String, List<PosvQuestion>> posvQAMap, Map<String, List<PosvQuestion>> posvQA12Map, boolean isSspSwissReCase) {
		...
		medicalQuestions.setQuestionSwpDrugsDetailAnswer(getMedicalQuestion12Answer(posvQA12Map, DRUG_DETAILS));
		...
		setConsumptionQuestions(medicalQuestions, baseMapper, posvQAMap, posvQA12Map, isSspSwissReCase);
	...
	private void setWeightChangeQuestionData(ProposalDetails proposalDetails, Map<String, Object> dataForDocument){
	...
	private AnnexureMedicalQuestions setAnnexureQuestions(AnnexureMedicalQuestions annexureQuestions, BaseMapper baseMapper, Map<String, List<PosvQuestion>> posvQAMap) {
        ...
        if(annexureQuestions.getQuesTobaccoAlcoholDrugAns().equalsIgnoreCase(AppConstants.NO)
        		&& annexureQuestions.getQuesTobaccoAlcoholDrugAnsInsured().equalsIgnoreCase(AppConstants.NO)) {
        	//
        }
	...
	private MedicalQuestions setConsumptionQuestions(MedicalQuestions medicalQuestions, BaseMapper baseMapper, Map<String, List<PosvQuestion>> posvQAMap, Map<String, List<PosvQuestion>> posvQA12Map, boolean isSspSwissReCase) {
//		if(!isSspSwissReCase){
			medicalQuestions.setQuestionTobaccoAnswer(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H19", "H19i"));
			medicalQuestions.setQuestionTobaccoSmokingAnswer(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H19", "H19i"));
			medicalQuestions.setQuestionTobaccoSmokingQuantityAnswer(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H19i", "H19i1")));
			medicalQuestions.setQuestionTobaccoSmokingFrequencyAnswer(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H19i", "H19i2")));
			medicalQuestions.setQuestionTobaccoEatingAnswer(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H19", "H19ii"));
			medicalQuestions.setQuestionTobaccoEatingQuantityAnswer(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, TOBACCO_GUTKA_PANMASALA, "H19ii1")));
			medicalQuestions.setQuestionTobaccoEatingFrequencyAnswer(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, TOBACCO_GUTKA_PANMASALA, "H19ii2")));
			medicalQuestions.setQuestionAlcoholAnswer(baseMapper.getMedicalQuestionAnswer(posvQAMap, "NA", "H20"));
			medicalQuestions.setQuestionAlcoholBeerAnswer(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H20", "H20i"));
			medicalQuestions.setQuestionAlcoholBeerQuantityAnswer(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, BEER, "H20i1")));
			medicalQuestions.setQuestionAlcoholBeerFrequencyAnswer(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, BEER, "H20i2")));
			medicalQuestions.setQuestionAlcoholWineAnswer(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H20", WINE));
			medicalQuestions.setQuestionAlcoholWineQuantityAnswer(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, WINE, "H20ii1")));
			medicalQuestions.setQuestionAlcoholWineFrequencyAnswer(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, WINE, "H20ii2")));
			medicalQuestions.setQuestionAlcoholHardLiquorAnswer(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H20", HARD_LIQUOR));
			medicalQuestions.setQuestionAlcoholHardLiquorQuantityAnswer(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, HARD_LIQUOR, "H20iii1")));
			medicalQuestions.setQuestionAlcoholHardLiquorFrequencyAnswer(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, HARD_LIQUOR, "H20iii2")));
			medicalQuestions.setQuestionDrugsAnswer(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H12", "H12C")));
			medicalQuestions.setQuestionDrugsDetailsAnswer(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H12C", DRUG_DETAILS)));
			// Insured
			medicalQuestions.setQuestionTobaccoAnswerInsured(baseMapper.getMedicalQuestionAnswer(posvQAMap, "NA", "H42"));
			medicalQuestions.setQuestionTobaccoSmokingAnswerInsured(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H42", "H42i"));
			medicalQuestions.setQuestionTobaccoSmokingQuantityAnswerInsured(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H42i", "H42i1")));
			medicalQuestions.setQuestionTobaccoSmokingFrequencyAnswerInsured(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H42i", "H42i2")));
			medicalQuestions.setQuestionTobaccoEatingAnswerInsured(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H42", TOBACCO_GUTKA_PANMASALA_INSURED));
			medicalQuestions.setQuestionTobaccoEatingQuantityAnswerInsured(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, TOBACCO_GUTKA_PANMASALA_INSURED, "H42ii1")));
			medicalQuestions.setQuestionTobaccoEatingFrequencyAnswerInsured(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, TOBACCO_GUTKA_PANMASALA_INSURED, "H42ii2")));
			medicalQuestions.setQuestionAlcoholAnswerInsured(baseMapper.getMedicalQuestionAnswer(posvQAMap, "NA", "H43"));
			medicalQuestions.setQuestionAlcoholBeerAnswerInsured(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H43", BEER_INSURED));
			medicalQuestions.setQuestionAlcoholBeerQuantityAnswerInsured(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, BEER_INSURED, "H43i1")));
			medicalQuestions.setQuestionAlcoholBeerFrequencyAnswerInsured(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, BEER_INSURED, "H43i2")));
			medicalQuestions.setQuestionAlcoholWineAnswerInsured(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H43", WINE_INSURED));
			medicalQuestions.setQuestionAlcoholWineQuantityAnswerInsured(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, WINE_INSURED, "H43ii1")));
			medicalQuestions.setQuestionAlcoholWineFrequencyAnswerInsured(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, WINE_INSURED, "H43ii2")));
			medicalQuestions.setQuestionAlcoholHardLiquorAnswerInsured(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H43", HARD_LIQUOR_INSURED));
			medicalQuestions.setQuestionAlcoholHardLiquorQuantityAnswerInsured(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, HARD_LIQUOR_INSURED, "H43iii1")));
			medicalQuestions.setQuestionAlcoholHardLiquorFrequencyAnswerInsured(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, HARD_LIQUOR_INSURED, "H43iii2")));
			medicalQuestions.setQuestionDrugsAnswerInsured(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H36", "H36C")));
			medicalQuestions.setQuestionDrugsDetailsAnswerInsured(Utility.ifEmptyThenNA(baseMapper.getMedicalQuestionAnswer(posvQAMap, "H36C", "H36Ci")));
			
			if (medicalQuestions.getQuestionTobaccoAnswer().equalsIgnoreCase(AppConstants.YES)
					|| medicalQuestions.getQuestionAlcoholAnswer().equalsIgnoreCase(AppConstants.YES)
					|| medicalQuestions.getQuestionDrugsAnswer().equalsIgnoreCase(AppConstants.YES)) {
				medicalQuestions.setQuestionTobaccoAlcoholDrugConsumption(AppConstants.YES);
			} else if (medicalQuestions.getQuestionTobaccoAnswer().equalsIgnoreCase(AppConstants.NA)
					&& medicalQuestions.getQuestionAlcoholAnswer().equalsIgnoreCase(AppConstants.NA)
					&& medicalQuestions.getQuestionDrugsAnswer().equalsIgnoreCase(AppConstants.NA)) {
				medicalQuestions.setQuestionTobaccoAlcoholDrugConsumption(AppConstants.NA);
			} else {
				medicalQuestions.setQuestionTobaccoAlcoholDrugConsumption(AppConstants.NO);
			}
			
			if (medicalQuestions.getQuestionTobaccoAnswerInsured().equalsIgnoreCase(AppConstants.YES)
					|| medicalQuestions.getQuestionAlcoholAnswerInsured().equalsIgnoreCase(AppConstants.YES)
					|| medicalQuestions.getQuestionDrugsAnswerInsured().equalsIgnoreCase(AppConstants.YES)) {
				medicalQuestions.setQuestionTobaccoAlcoholDrugConsumptionInsured(AppConstants.YES);
			} else if (medicalQuestions.getQuestionTobaccoAnswerInsured().equalsIgnoreCase(AppConstants.NA)
					&& medicalQuestions.getQuestionAlcoholAnswerInsured().equalsIgnoreCase(AppConstants.NA)
					&& medicalQuestions.getQuestionDrugsAnswerInsured().equalsIgnoreCase(AppConstants.NA)) {
				medicalQuestions.setQuestionTobaccoAlcoholDrugConsumptionInsured(AppConstants.NA);
			} else {
				medicalQuestions.setQuestionTobaccoAlcoholDrugConsumptionInsured(AppConstants.NO);
			}
