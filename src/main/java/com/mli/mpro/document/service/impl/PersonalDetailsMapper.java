...
public class PersonalDetailsMapper {
	...
	private static final String DUTIES_INFO = "dutiesInfo";
   ...
   private static final String HIGH_SCHOOL = "High School";
	...
	try {
        ...
        String residenceForTax = Objects.nonNull(proposerBasicDetails.getResidenceForTax()) ? proposerBasicDetails.getResidenceForTax() : StringUtil.EMPTY_STRING;
				...
				NatureofDuties=checkIfValueIsPresent(proposerEmploymentDetails,DUTIES_INFO);
				if (isNeoOrAggregator
        ...
        if ((AppConstants.DEPENDENT.equalsIgnoreCase(formType) || isProposerPresent || (proposalDetails.getProductDetails() != null
				...
				&& Utility.isProductSGPPJL(proposalDetails)
		...
		if (AppConstants.ANNUITY_PRODUCTS.contains(proposalDetails.getProductDetails().get(0).getProductInfo().getProductId())
			...
			insuredfatherMiddleName = getName(insuredfatherName,MIDDLENAME);
			insurednationality = insuredBasicDetails.getNationalityDetails().getNationality();
			if(!isNeoOrAggregator && Utility.isChannelYBLNIS(proposalDetails) && productId.equalsIgnoreCase(AppConstants.SWAG_PP_PRODUCT_ID)) {
				insuredformattedDob = Utility.dateFormatter(productInfo.getSecondAnnuitantDob(), "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'", DD_MM_YYYY_HYPEN_FORMAT);
			} else {
				insuredformattedDob = Utility.stringAnnuityFormDateFormatter(productInfo.getSecondAnnuitantDob(),proposalDetails.getChannelDetails().getChannel());
			}
			setDataForIndustryInsured(dataVariables,
					proposalDetails.getEmploymentDetails().getPartiesInformation().get(1).getPartyDetails().getIndustryDetails());
			if (AppConstants.WHOLE_LIFE_INCOME.equalsIgnoreCase(proposalDetails.getProductDetails().get(0).getProductInfo().getVariant())) {
		...
		if(!StringUtils.isEmpty(partyInfoList.get(0).getBSE500Company()) && partyInfoList.get(0).getBSE500Company()!=null && StringUtils.isEmpty(employerName)){
		...
		if (Objects.nonNull(proposerEmploymentDetails)) {
			if (isNeoOrAggregator) {
				...
				exitingEIA = !StringUtils.isEmpty(proposerEmploymentDetails.getExistingEIANumber())
						? proposerEmploymentDetails.getExistingEIANumber() : AppConstants.NO;
				existingRepoName = !StringUtils.isEmpty(proposerEmploymentDetails.getExistingEIANumberRepositoryName())
						? Utility.getFullRepositoryName(proposerEmploymentDetails.getExistingEIANumberRepositoryName()) : AppConstants.BLANK;
			} else {

				exitingEIA = !StringUtils.isEmpty(proposerEmploymentDetails.getExistingEIANumber()) ? AppConstants.YES : AppConstants.NO;
			}
			...
			preferredInsuranceRepository = !StringUtils.isEmpty(proposerEmploymentDetails.getPreferredInsuranceRepositoryName()) ? AppConstants.YES : AppConstants.NO;
		}
		...
		if (!StringUtils.isEmpty(nominee.getFatherOrHusbandName()) && nominee.getFatherOrHusbandName().contains(" ")) {
		...
		String isThisSwagEliteProduct = AppConstants.SWAG_ELITE_PRODUCT_ID.equalsIgnoreCase(productId) ? AppConstants.YES : AppConstants.NO;
		...
		dataVariables.put("isThisSwagEliteProduct",isThisSwagEliteProduct);
	    ...
	    if (isNeoOrAggregator && !AppConstants.YBL.equalsIgnoreCase(proposalDetails.getBankJourney())) {
			...
			if(Utility.isSSPJLProduct(proposalDetails)){
				dataVariables.put("insurednavyReflexive1", proposalDetails.getEmploymentDetails().getPartiesInformation().get(1).getPartyDetails().getRank());
			}
    ...
    private String checkIfValueIsPresent(Object obj , String type) {
    	...
    	switch (type) {
			...
			case DUTIES_INFO:{
				EmploymentDetails proposerEmploymentDetails = (EmploymentDetails) obj;
				return StringUtils.isEmpty(proposerEmploymentDetails.getPartiesInformation().get(0).getPartyDetails().getNatureOfDuties()) ? "NA"
						: proposerEmploymentDetails.getPartiesInformation().get(0).getPartyDetails().getNatureOfDuties();
			}
			case NRI_DETAILS:{
				BasicDetails insuredBasicDetails = (BasicDetails)obj;
