...
//FUL-238597
@Component
public class ShorterJourney implements OtpFlowService {
    private final EmailSmsService emailSmsService;
    private OtpUtility otpUtility;
    private OauthTokenRepository oauthTokenRepository;
    private static final Logger logger= LoggerFactory.getLogger(ShorterJourney.class);

    @Autowired
    public ShorterJourney(EmailSmsService emailSmsService, OtpUtility otpUtility, OauthTokenRepository oauthTokenRepository) {
    ...
    public void setSmsPayload(RequestPayload smsPayload, Object requestObject, String otp, String proposerName) {
    	String messageBody = AppConstants.SHORTER_JOURNEY_OTP_MSG_BODY;
        Payload payload = (Payload) requestObject;
        if(payload != null) {
        	smsPayload.setCustomerName((payload.getDisabilityDeclaration() != null && payload.getDisabilityDeclaration().getCustomerName() != null) ? payload.getDisabilityDeclaration().getCustomerName() : "");
            smsPayload.setPolicyNumber(payload.getPolicyNumber() != null ? payload.getPolicyNumber() : "");
            smsPayload.setMessageTo((payload.getDisabilityDeclaration() != null && payload.getDisabilityDeclaration().getMobileNo() != null) ? payload.getDisabilityDeclaration().getMobileNo() : "");
            smsPayload.setType(payload.getOtpDeliveryMethod() != null ? payload.getOtpDeliveryMethod() : "");
        ...
        if(Boolean.TRUE.equals(FeatureFlagUtil.isFeatureFlagEnabled(AppConstants.NEW_SMS_API))){
    	   ...
    	   smsPayload.setMessageText(messageBody.replace("<otp>", otp));        
    }
}