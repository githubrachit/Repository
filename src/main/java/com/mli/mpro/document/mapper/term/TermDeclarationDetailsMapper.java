...
import static com.mli.mpro.productRestriction.util.AppConstants.AGENT_ADVISOR_NAME;
import static com.mli.mpro.productRestriction.util.AppConstants.BLANK;
import static com.mli.mpro.productRestriction.util.AppConstants.INFLUENCER_CHANNEL_38;
...
@Service
public class TermDeclarationDetailsMapper {
	@Autowired
    ...
    private static final Logger logger = LoggerFactory.getLogger(TermDeclarationDetailsMapper.class);
	...
	try {
		...
		boolean isTradAndNotNeoAggregator = !isNeoOrAggregator && AppConstants.TRADITIONAL.equalsIgnoreCase(proposalDetails.getProductDetails().get(0).getProductType());
		String pdfVersionNumber = isTradAndNotNeoAggregator ? AppConstants.PF_VERSION_TERM : AppConstants.PF_VERSION;
		pdfVersion = isTradAndNotNeoAggregator ? AppConstants.PDF_VERSION_TERM : AppConstants.PDF_VERSION;
		...
		if (AppConstants.CHANNEL_AXIS.equalsIgnoreCase(channel)) {
			...
			versionDetails = StringUtils.join(AppConstants.PF_VERSION_START_TERM, "AXIS/", pdfVersion,
					pdfVersionNumber);
			isAgencyOrAxisOrYblChannel = "YES";
			...
			versionDetails = StringUtils.join(AppConstants.PF_VERSION_START_TERM, channelName.get(channel), "/",
					pdfVersion
					, pdfVersionNumber);
		} else {
			agentName = Objects.nonNull(proposalDetails.getSourcingDetails()) ?
			...
			versionDetails = StringUtils.join(AppConstants.PF_VERSION_START_TERM, pdfVersion, pdfVersionNumber);
		}
	    ...
	    if ((AppConstants.CHANNEL_NEO.equalsIgnoreCase(channel)
			...
			dataVariables.put("date", Utility.getDateOnTheBasisOfRateChange(proposalDetails, false));
	...
	private  boolean compareWipCaseDate(long transactionId,Date createdTime,String key) {
		try {
			logger.info("Comparing WIP Case Date for TRAD transactionId: {}",transactionId);
			...
			logger.error("Error occurred while comparing WIP Case Date for TRAD transactionId: {}",transactionId,e);
			return false;
		}
	}
