...
@Service
public class CoverageDetailsMapper {

    private static final Logger logger = LoggerFactory.getLogger(CoverageDetailsMapper.class);
	...
	try {
		...
		if(Objects.nonNull(proposalDetails.getQuotePayload())
				&& Objects.nonNull(proposalDetails.getQuotePayload().getApplication())
				&& Objects.nonNull(proposalDetails.getQuotePayload().getApplication().getQuote())
				&& Objects.nonNull(proposalDetails.getQuotePayload().getApplication().getQuote().getProductsDetails())
				&& Objects.nonNull(proposalDetails.getQuotePayload().getApplication().getQuote().getProductsDetails().get(0))
				&& Objects.nonNull(proposalDetails.getQuotePayload().getApplication().getQuote().getProductsDetails().get(0).getProductsList())
				&& Objects.nonNull(proposalDetails.getQuotePayload().getApplication().getQuote().getProductsDetails().get(0).getProductsList().get(0))
				&& Objects.nonNull(proposalDetails.getQuotePayload().getApplication().getQuote().getProductsDetails().get(0).getProductsList().get(0).getQuoteDetails())) {
			quoteDetails = proposalDetails.getQuotePayload().getApplication().getQuote().getProductsDetails().get(0).getProductsList().get(0).getQuoteDetails();
		}
		...
		isNeo = AppConstants.CHANNEL_NEO.equalsIgnoreCase(proposalDetails.getChannelDetails().getChannel());
		String isStaffOrSellerDiscountApplicable = "";
		if((isNeoOrAggregator)){
			isStaffOrSellerDiscountApplicable =quoteDetails.getIsStaffOrSellerDiscountApplicable();
		}
		...
		if ((isNeoOrAggregator)) {
			if(checkingBankDetailsNullValue(proposalDetails)) {
	    ...
	    if (isNeoOrAggregator) {
            ...
            String pcbRiderSelection = Utility.isNotNullOrEmpty(productDetails.getProductInfo().getPcbRiderSelection()) ? productDetails.getProductInfo().getPcbRiderSelection() : NA;
            dataVariables.put("pcbRiderSelection", pcbRiderSelection);
	    ...
	    String pcb = Utility.evaluateConditionalOperation(AppConstants.SWIP.equals(productId) || AppConstants.SWAG.equals(productId) || AppConstants.SWAG_PAR.equals(productId)|| AppConstants.SWAG_ELITE_PRODUCT_ID.equals(productId),Optional.ofNullable(StringUtils.capitalize(productDetails.getProductInfo().getPcb())).orElse(AppConstants.BLANK), "NA");
	   ...
	   //FUL2-7517 Smart Wealth Plan in mPRO - PF Changes
		...
		String incomePayoutFrequency = getIncomePayoutFrequency(productDetails, isNeoOrAggregator);
		...
		//FUL2-11860 Check for SWP as wll
		//FUL2-33312 adding SAP product 
		// FUL2-118606/FUL2-119208 inclusion of SWAG_PAR product
		if(Utility.or(AppConstants.SWP_PRODUCTCODE.equalsIgnoreCase(productId),AppConstants.SAP_PRODUCT_ID.equalsIgnoreCase(productId),AppConstants.SJB_PRODUCT_ID.equalsIgnoreCase(productId), AppConstants.SMTP_PRODUCT_ID.equalsIgnoreCase(productId),
					AppConstants.SWAG.equalsIgnoreCase(productId), AppConstants.SWAG_PAR.equalsIgnoreCase(productId), AppConstants.OTP_PRODUCT_ID.equalsIgnoreCase(productId), AppConstants.SSP_PRODUCT_ID.equalsIgnoreCase(productId),
				AppConstants.SWAG_ELITE_PRODUCT_ID.equalsIgnoreCase(productId), AppConstants.SWIP.equalsIgnoreCase(productId))) {
			...
			dataVariables.put("smartTerm", AppConstants.OTP_PRODUCT_ID.equalsIgnoreCase(productId) || AppConstants.SMTP_PRODUCT_ID.equalsIgnoreCase(productId) || AppConstants.SSP_PRODUCT_ID.equalsIgnoreCase(productId));
			dataVariables.put("pBoPpt", Utility.or(AppConstants.SJB_PRODUCT_ID.equalsIgnoreCase(productId),AppConstants.SAP_PRODUCT_ID.equalsIgnoreCase(productId), AppConstants.OTP_PRODUCT_ID.equalsIgnoreCase(productId),
					AppConstants.SWAG.equalsIgnoreCase(productId),AppConstants.SWAG_PAR.equalsIgnoreCase(productId), AppConstants.SWP_PRODUCTCODE.equalsIgnoreCase(productId),AppConstants.SMTP_PRODUCT_ID.equalsIgnoreCase(productId),Utility.isSSPJointLife(productDetails)));
			dataVariables.put("deathBenefit",Utility.evaluateConditionalOperation(
					(productDetails.getProductInfo().getDeathBenefit()==null  || StringUtils.isEmpty(productDetails.getProductInfo().getDeathBenefit()) ) , AppConstants.NA , productDetails.getProductInfo().getDeathBenefit()));
			dataVariables.put("lifeStageEvent",Utility.evaluateConditionalOperation(  productDetails.getProductInfo().getDeathBenefit()==null ||StringUtils.isEmpty(productDetails.getProductInfo().getLifeStageEventBenefit()) , AppConstants.NA
					, productDetails.getProductInfo().getLifeStageEventBenefit()));
		}
	    ...
	    // FUL2-118606/FUL2-119208 inclusion of SWAG_PAR product
	    isSWPSJBSWAGSWAG_PAR = Utility.or(AppConstants.SJB_PRODUCT_ID.equalsIgnoreCase(productId),AppConstants.SWP_PRODUCTCODE.equalsIgnoreCase(productId), AppConstants.SWAG.equalsIgnoreCase(productId), AppConstants.SWAG_PAR.equalsIgnoreCase(productId)) ? Boolean.TRUE : Boolean.FALSE;

		isSmartWealthPlan = AppConstants.SWP_PRODUCTCODE.equalsIgnoreCase(productId) ? Boolean.TRUE : Boolean.FALSE;
		...
		// FUL2-118606/FUL2-119208 inclusion of SWAG_PAR product
		if(Utility.or(AppConstants.SJB_PRODUCT_ID.equalsIgnoreCase(productId),AppConstants.SWAG.equalsIgnoreCase(productId),AppConstants.SWAG_PAR.equalsIgnoreCase(productId),AppConstants.SWP_PRODUCTCODE.equalsIgnoreCase(productId)) && (AppConstants.LONGTERM_INCOME.equalsIgnoreCase(variant) || AppConstants.SHORTTERM_INCOME.equalsIgnoreCase(variant))
				&& !StringUtils.isEmpty(annualIncomeAmt)) {
			 incomePayout = roundOffValue(Double.valueOf(annualIncomeAmt));
			 illusIncomePayoutFrequency = roundOffValue(Double.valueOf(productDetails.getProductInfo().getProductIllustrationResponse().getIncomePayoutFrequency()));
			 incomePayout = incomePayout * ((int)illusIncomePayoutFrequency);
			 isIncomePayout = Boolean.TRUE;
		}
		...
		if(StringUtils.isNotEmpty(productDetails.getProductType()) && StringUtils.isNotEmpty(productDetails.getProductInfo().getProductId()) && NEO_STPP_PRODUCT_TYPE.equalsIgnoreCase(productDetails.getProductType()) && STPP_PRODUCT_PREMIUM_BACK_OPTION.contains(productDetails.getProductInfo().getProductId())){
			premiumBackOption = AppConstants.YES;
		}
		...
		//FUL2-196217_SWAG_Elite_Pf_changes
		String deathBenefitMultiple = StringUtils.EMPTY;
		 deathBenefitMultiple = productDetails.getProductInfo() != null
				 && productDetails.getProductInfo().getDeathBenefitMultiple()!=null
				? productDetails.getProductInfo().getDeathBenefitMultiple()
				: deathBenefitMultiple;

		if(SWAG_ELITE_PRODUCT_ID.equalsIgnoreCase(productId)){
			coverageTerm = WHOLE_LIFE;
			sumAssured = AppConstants.NA;
			incomePayoutFrequency = AppConstants.NA;
		}
        ...
        dataVariables.put("incomeCover", StringUtils.isEmpty(productDetails.getProductInfo().getIncomeCover())
                ? NA : productDetails.getProductInfo().getIncomeCover());
        dataVariables.put("sumAssuredBooster", StringUtils.isEmpty(productDetails.getProductInfo().getSumAssuredBooster())
                ? NA : Utility.convertToYesOrNo(productDetails.getProductInfo().getSumAssuredBooster()));
	   ...
	   // FUL2 -144663
	    ...
	    // FUL2-118606/FUL2-119208 inclusion of SWAG_PAR product
	    dataVariables.put("SWPSJBSWAGSWAG_PARplan", isSWPSJBSWAGSWAG_PAR);
		...
		dataVariables.put("isStaffOrSellerDiscountApplicable", isStaffOrSellerDiscountApplicable);
		dataVariables.put("deathBenefitMultiple",deathBenefitMultiple);
	  //FUL2-46299_52545
	...
	try {
		...
		dataVariables.put(AppConstants.TRANSACTION_NUMBER, transactionNumber);
				...
				AppConstants.SWAGPP.equalsIgnoreCase(proposalDetails.getProductDetails().get(0).getProductInfo().getProductId()))) {
            amount = Utility.setPremiumAmount(proposalDetails);
	    ...
	    dataVariables.put("amountWords", amountInWords);
	    dataVariables.put("amount", String.format("%.2f", amount));
	...
	private void setPremiumPayingTerm(List<RiderDetails> riderDetails, int riderCount,
		...
		} else if (Utility.isAddRider(riderName)) {
			details.setPremiumPayingTerm(riderDetails.get(riderCount).getRiderPpt());
		}else {
	...
	private String getIncomePayoutFrequency(ProductDetails productDetails, boolean isNeoOrAggregator) {
		String incomePayoutFrequency = productDetails.getProductInfo().getIncomePayoutFrequency();
		if (!isNeoOrAggregator && (SWAG.equalsIgnoreCase(productDetails.getProductInfo().getProductId())
				&& LIMITED_PAY.equalsIgnoreCase(productDetails.getProductInfo().getPremiumType())
				&& EARLY_WEALTH.equalsIgnoreCase(productDetails.getProductInfo().getVariant()))
				|| SWAG_PAR.equalsIgnoreCase(productDetails.getProductInfo().getProductId())
				|| SWIP.equalsIgnoreCase(productDetails.getProductInfo().getProductId())) {
			String incomePayoutOption = productDetails.getProductInfo().getIncomePayoutOption();
			if(!StringUtils.isEmpty(incomePayoutOption)) {
				return String.join(" - ", incomePayoutFrequency, StringUtils.capitalize(incomePayoutOption.toLowerCase()));
			}
		}
